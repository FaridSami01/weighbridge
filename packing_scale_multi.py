...
class BaykonMachine:
    ...
    def __init__(self, name, port, baudrate=9600, division=100, 
                 ...
                 on_bag_complete=None, machine_id=None, db_manager=None):
        self.name = name
        self.port = port
        ...
        self.machine_id = machine_id or name.lower().replace(' ', '_').replace('-', '_')
        ...
        # FIX: Track last time data was actually received, used by is_online()
        self._last_data_time = None
        self._data_timeout = 10.0  # seconds before considered offline
        ...
        self.patterns = [
            ...
            # FIX: Format 3 - BX30 specific pattern anchored more tightly.
            # BX30 typically sends weight as a standalone field, e.g. "  04987  " or
            # a line like "W 04987" or just "04987\r\n".
            # Using word boundaries and requiring the number to stand alone to avoid
            # matching timestamps, checksums, or header bytes.
            ...
            try:
                ...
                if self.serial and self.serial.is_open:
                    if self.serial.in_waiting:
                        ...
                        # FIX: Record actual data receipt time
                ...
                try:
                    ...
                    if self.detected_format is None:
                        ...
                        format_names = ["Format 1 (t01 3990)", 
                                      "Format 2 (l80 4009)", 
                                      "Format 3 (BX30 standalone number)"]
        ...
        if weight_kg <= self.EMPTY_THRESHOLD and self.state == "FULL":
            ...
            if self.db_manager:
                try:
                    self.db_manager.save_bag(
                        ...
                        machine_name=self.name,
                        bag_number=self.bag_count,
                        weight_kg=self.last_full_weight,
                        operator='SYSTEM'
                    )
            ...
            if self.on_bag_complete:
                try:
                    ...
                    print(f"   ⚠️  Callback save failed: {e}")
    ...
    def is_online(self):
        """
        FIX: Check if scale is truly online — port must be open AND data must
        have been received within the last _data_timeout seconds.
        Previously this only checked serial.is_open, which returned True even
        when the port was open but the scale was silent (wrong baud, bad cable).
        """
        try:
            port_open = self.is_running and self.serial is not None and self.serial.is_open
            if not port_open:
                return False
            # If we have never received any data yet allow a grace period
            # (first _data_timeout seconds after connect) so startup doesn't
            # immediately flag the machine as offline.
    ...
    def load_today_stats(self):
        ...
        if self.db_manager:
            try:
                ...
                # Convert database format to session_bags format
                self.session_bags = []
                for bag in stats['bags']:
                    self.session_bags.append({
                        'bag_number': bag['bag_number'],
                        'weight': float(bag['weight_kg']),
                        'timestamp': datetime.now(),  # Use current date with time from DB
                        'machine_name': self.name,
                        'bag_size': self.bag_size,
                        'product': self.product
                    })
                ...
                print(f"⚠️  {self.name}: Could not load stats from database: {e}")
...
class MultiMachineManager:
    ...
    def __init__(self, on_bag_complete=None, db_manager=None):
        self.machines = {}
        self.on_bag_complete = on_bag_complete  # Callback for auto-save
        self.db_manager = db_manager  # Database manager
    ...
    def _initialize_machines(self):
        ...
        # FIX: Default for bran corrected from COM4 to COM3 to match app.py comment
        import os
        ...
        try:
            self.machines['bran_40kg'] = BaykonMachine(
                ...
                port=port_bran,
                baudrate=9600,
                division=100,        # BX3 sends raw value in 10g units → divide by 100 → kg
                full_threshold=38.0,
                empty_threshold=2.0,
                bag_size=40,
                product="Coarse Bran",
                on_bag_complete=self.on_bag_complete,
                machine_id="bran_40kg",
        ...
        try:
            self.machines['flour_50kg_bxf3'] = BaykonMachine(
                ...
                port=port_flour1,
                baudrate=9600,
                division=100,        # BXf3 sends raw value in 10g units → divide by 100 → kg
                full_threshold=48.0,
                empty_threshold=2.0,
                bag_size=50,
                product="Flour",
                on_bag_complete=self.on_bag_complete,
                machine_id="flour_50kg_bxf3",
                db_manager=self.db_manager
            )
            time.sleep(0.5)  # Wait for thread to stabilize
        except Exception as e:
        ...
        # FIX: division=100 explicitly set (was missing, inheriting default but
        # now clearly documented). The BX30 outputs weight in 10g increments,
        # e.g. raw value 5000 = 50.00 kg. Dividing by 100 gives correct kg.
        # If your BX30 firmware sends a different scale factor, adjust this value:
        #   division=10  → raw 500  = 50.0 kg  (100g increments)
        #   division=100 → raw 5000 = 50.0 kg  (10g increments)  ← default
        #   division=1   → raw 50   = 50.0 kg  (1kg increments, unlikely)
        try:
            self.machines['flour_50kg_bx30'] = BaykonMachine(
                name="BX30 - Flour 50kg",
                port=port_flour2,
                baudrate=9600,
                division=100,        # FIX: explicitly set; was missing before
                full_threshold=48.0,
                empty_threshold=2.0,
                bag_size=50,
                product="Flour",
                on_bag_complete=self.on_bag_complete,
                machine_id="flour_50kg_bx30",
                db_manager=self.db_manager
            )
            time.sleep(0.5)  # Wait for thread to stabilize
        except Exception as e:
        ...
        if online_count == 0:
            ...
            print("      PACKING_PORT_BRAN   (default COM3 - BX3)")
            print("      PACKING_PORT_FLOUR1 (default COM9 - BXf3)")
            print("      PACKING_PORT_FLOUR2 (default COM5 - BX30)")
            print()
