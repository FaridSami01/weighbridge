...
def get_translations():
    """Helper function to get translations dictionary"""
    lang = session.get("lang", "en")
...
def intake():
    ...
    if request.method == "POST":
        try:
            ...
            truck_number = request.form.get("truck", "").strip()  # Form sends 'truck', DB has 'truck_number'
            trailer_number = request.form.get("trailer_number", "").strip()
            ...
            driver_name = request.form.get("driver", "")  # Form sends 'driver', DB has 'driver_name'
            
            # WEIGHT FIELDS
            gross = float(request.form.get("gross", 0))
            first_weight = float(request.form.get("first_weight", 0))
            ...
            with get_db() as conn:
                ...
                # Insert using ACTUAL database column names
                cur.execute("""
                    INSERT INTO intake 
                    (vendor_id, truck_number, trailer_number, wheat_type, 
                     carrier_name, driver_name, governorate, load_type,
                     fine_bran_given, gross_weight, 
                     captured_by, captured_at) 
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NOW())
                """, (
                    vendor_id, truck_number, trailer_number, wheat_type,
                    carrier_name, driver_name, governorate, load_type,
                    first_weight, gross,  # fine_bran_given is used as first_weight
                    session.get("user")
                ))
    ...
    try:
        ...
        return render_template("intake.html", vendors=vendors, t=get_translations())
    except Exception as e:
        print(f"Error loading vendors: {e}")
        return render_template("intake.html", vendors=[], t=get_translations())

@app.route("/second_weighing", methods=["GET", "POST"])
@login_required
def second_weighing():
    """
    Second weighing page - GET shows form, POST saves data
    """
    if request.method == "POST":
        # POST - Save second weighing
        try:
            # Get form data
            intake_id = request.form.get('intake_id')
            second_weight = request.form.get('second_weight')
            ...
            with get_db() as conn:
                ...
                # Get first weight - use fine_bran_given (first_weight column)
                cursor.execute("""
                    SELECT fine_bran_given as first_weight FROM intake WHERE id = %s
                """, (intake_id,))
                result = cursor.fetchone()
                
                if not result:
                    flash('Intake entry not found', 'danger')
    ...
    try:
        with get_db() as conn:
            ...
            # Get intake entries - use actual column names from your database
            # NOTE: fine_bran_given >= 0 allows testing with no weight on scale
            cursor.execute("""
                SELECT 
                    i.id,
                    DATE_FORMAT(i.captured_at, '%Y-%m-%d') as date,
                    v.name as vendor_name,
                    i.truck_number as truck,
                    i.trailer_number,
                    i.wheat_type,
                    i.carrier_name,
                    i.driver_name,
                ...
                WHERE i.fine_bran_given IS NOT NULL 
                  AND i.fine_bran_given >= 0
                  AND (i.second_weight IS NULL OR i.second_weight = 0)
                ORDER BY i.captured_at DESC, i.id DESC
                LIMIT 100
            """)
            
            recent_intakes = cursor.fetchall()
            
            # Get vendors for any dropdowns if needed
            cursor.execute("SELECT id, name FROM vendors ORDER BY name")
            vendors = cursor.fetchall()
            
        return render_template(
            'second_weighing.html',
        ...
        print(f"Error loading second weighing page: {e}")
        import traceback
        traceback.print_exc()
        return render_template(
            'second_weighing.html',
...
@app.route("/weighing/receipt/<int:intake_id>")
@login_required
def generate_weighing_receipt(intake_id):
    """Generate PDF receipt for completed weighing with Arabic support"""
    try:
        from reportlab.lib.pagesizes import A4
        ...
        # ============== HEADER ==============
        pdf.setFillColor(HexColor('#1e3c72'))
        pdf.rect(0, height - 50*mm, width, 50*mm, fill=1)
        
        # Company name
        pdf.setFillColorRGB(1, 1, 1)
        pdf.setFont("Helvetica-Bold", 24)
        pdf.drawCentredString(width/2, height - 25*mm, "Modern Engineer Mills")
        
        # Arabic name (transliterated since we can't render Arabic fonts)
        pdf.setFont("Helvetica", 18)
        pdf.drawCentredString(width/2, height - 35*mm, "Matahin Al Muhandis Al Haditha")
        
        pdf.setFont("Helvetica-Bold", 14)
        pdf.drawCentredString(width/2, height - 45*mm, f"Weighing Receipt No: {data['id']}")
        
        current_y = height - 60*mm
        
        # ============== CLIENT INFORMATION ==============
        pdf.setFillColor(HexColor('#2c3e50'))
        pdf.rect(margin, current_y - 5*mm, width - 2*margin, 8*mm, fill=1)
        
        pdf.setFillColorRGB(1, 1, 1)
        pdf.setFont("Helvetica-Bold", 12)
        pdf.drawString(margin + 5*mm, current_y - 2*mm, "Client Information")
        ...
        line_height = 7*mm
        label_x = margin + 5*mm
        value_x = width - margin - 5*mm
        
        details = [
            ("Client Name (Ism Al-Ameel):", data['vendor_name'] or '-'),
            ("Truck Number (Raqm Al-Sayara):", data['truck_number'] or '-'),
            ("Trailer Number (Raqm Al-Maqtoora):", data['trailer_number'] or '-'),
            ("Governorate (Al-Muhafadha):", data['governorate'] or '-'),
            ("Load Type (Al-Homola):", data['load_type'] or '-'),
            ("Number of Bags (Adad Al-Ashwala):", str(data['num_bags'] or 0)),
            ("Bag Weight (Wazn Al-Shwal):", f"{data['bag_weight'] or 0:.2f} kg"),
        ]
        ...
        for label, value in details:
            ...
            pdf.setFont("Helvetica", 10)
            pdf.drawRightString(value_x, current_y, str(value))
        ...
        pdf.setFillColorRGB(1, 1, 1)
        pdf.setFont("Helvetica-Bold", 12)
        pdf.drawString(margin + 5*mm, current_y - 2*mm, "Weight Details")
        ...
        pdf.drawString(label_x, current_y, "First Weighing (Al-Wazna Al-Oula):")
        pdf.setFont("Helvetica-Bold", 12)
        pdf.drawRightString(value_x, current_y, f"{data['first_weight'] or 0:.2f} kg")
        
        current_y -= 6*mm
        pdf.setFont("Helvetica", 9)
        entry_time = data['captured_at'].strftime('%Y-%m-%d %H:%M:%S') if data['captured_at'] else '-'
        pdf.drawString(label_x, current_y, f"Date & Time (Al-Tarekh wa Al-Waqt): {entry_time}")
        
        current_y -= 10*mm
        
        # Second weighing
        pdf.setFont("Helvetica-Bold", 11)
        pdf.drawString(label_x, current_y, "Second Weighing (Al-Wazna Al-Thaniya):")
        pdf.setFont("Helvetica-Bold", 12)
        pdf.drawRightString(value_x, current_y, f"{data['second_weight'] or 0:.2f} kg")
        
        current_y -= 6*mm
        pdf.setFont("Helvetica", 9)
        exit_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        pdf.drawString(label_x, current_y, f"Date & Time (Al-Tarekh wa Al-Waqt): {exit_time}")
        ...
        pdf.setStrokeColor(HexColor('#3498db'))
        pdf.setLineWidth(2)
        pdf.rect(margin, current_y - box_height, width - 2*margin, box_height, fill=1, stroke=1)
        ...
        pdf.setFillColorRGB(0, 0, 0)
        pdf.setFont("Helvetica-Bold", 11)
        pdf.drawString(label_x, current_y, "Total Weight (Ijmali Al-Wazn):")
        pdf.setFont("Helvetica-Bold", 12)
        total_weight = abs((data['first_weight'] or 0) - (data['second_weight'] or 0))
        pdf.drawRightString(value_x, current_y, f"{total_weight:.2f} kg")
        
        current_y -= 9*mm
        
        # Code Weight
        pdf.setFont("Helvetica-Bold", 11)
        pdf.drawString(label_x, current_y, "Code Weight (Kod Al-Wazn):")
        pdf.setFont("Helvetica-Bold", 12)
        pdf.drawRightString(value_x, current_y, f"{data['code_weight'] or 0:.2f} kg")
        
        current_y -= 14*mm
        
        # Net Weight - Highlighted
        pdf.setFillColor(HexColor('#1e3c72'))
        pdf.rect(margin, current_y - 10*mm, width - 2*margin, 14*mm, fill=1)
        
        pdf.setFillColorRGB(1, 1, 1)
        pdf.setFont("Helvetica-Bold", 16)
        pdf.drawCentredString(width/2, current_y - 5*mm, 
                             f"NET WEIGHT (AL-SAFI): {data['net_weight'] or 0:.2f} kg")
        ...
        pdf.setFillColorRGB(0, 0, 0)
        pdf.setFont("Helvetica-Bold", 10)
        pdf.drawCentredString(margin + col_width/2, current_y, "Receiving Officer")
        pdf.setFont("Helvetica", 9)
        pdf.drawCentredString(margin + col_width/2, current_y - 5*mm, "Muwadaf Al-Istilam")
        pdf.line(margin + 10*mm, current_y - 20*mm, margin + col_width - 10*mm, current_y - 20*mm)
        pdf.setFont("Helvetica", 8)
        pdf.drawCentredString(margin + col_width/2, current_y - 23*mm, "Signature")
        
        # Receiver
        pdf.setFont("Helvetica-Bold", 10)
        pdf.drawCentredString(margin + col_width + col_width/2, current_y, "Receiver (Al-Mustalam)")
        pdf.setFont("Helvetica", 9)
        receiver_name = data['receiver_name'] or '_______________'
        pdf.drawCentredString(margin + col_width + col_width/2, current_y - 5*mm, receiver_name)
        pdf.line(margin + col_width + 10*mm, current_y - 20*mm, 
                margin + col_width*2 - 10*mm, current_y - 20*mm)
        pdf.setFont("Helvetica", 8)
        pdf.drawCentredString(margin + col_width + col_width/2, current_y - 23*mm, "Signature")
        
        # Official Stamp
        pdf.setFont("Helvetica-Bold", 10)
        pdf.drawCentredString(margin + col_width*2 + col_width/2, current_y, "Official Stamp")
        pdf.setFont("Helvetica", 9)
        pdf.drawCentredString(margin + col_width*2 + col_width/2, current_y - 5*mm, "Al-Khatm Al-Rasmi")
        pdf.rect(margin + col_width*2 + col_width/2 - 20*mm, current_y - 22*mm, 40*mm, 20*mm, stroke=1)
        ...
        pdf.drawString(margin, current_y, f"Printed on / Taba'a fi: {print_time}")
        ...
        return send_file(
            ...
            as_attachment=False,
            download_name=f'weighing_receipt_{intake_id}.pdf'
        )
...
# ============================================================================
# API Route to get specific intake entry by ID
# ============================================================================
...
def get_intake(intake_id):
    """
    Get specific intake entry for loading into second weighing
    """
    try:
        with get_db() as conn:
            cursor = conn.cursor(dictionary=True)
            ...
            if intake:
                ...
                return jsonify({
                    'success': False,
                    'error': 'Intake entry not found'
                }), 404
                
    except Exception as e:
        print(f"Error fetching intake {intake_id}: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
@login_required
def intake_history():
    """Intake history page with advanced filtering"""
    try:
        with get_db() as conn:
            cursor = conn.cursor(dictionary=True)
            
            # Get all intake records with correct column names
            cursor.execute("""
                SELECT 
                    i.id,
                    i.captured_at,
        ...
        print(f"Intake history error: {e}")
        import traceback
        traceback.print_exc()
        return render_template(
            "intake_history.html", 
